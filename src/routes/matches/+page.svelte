<script lang="ts">
	...
	const opponentId = (match: (typeof data.matches)[number]) =>
		match.playerOneMemberId === data.member.id ? match.playerTwoMemberId : match.playerOneMemberId;
	const myMatches = $derived(
		data.matches.filter(
			(match) =>
...
<section class="space-y-12">
	<div class="flex flex-wrap items-end justify-between gap-4">
		<div>
			<h1 class="font-serif text-4xl font-black text-agp-ink sm:text-5xl">Partidas</h1>
			<p class="mt-2 text-xl text-agp-muted">
				Confirmações, histórico e tudo que envolve teu nome.
			</p>
		</div>
	...
	<Card class="overflow-hidden">
		<div class="border-b border-agp-border bg-stone-100/70 px-5 py-4 lg:px-8">
			<h2 class="text-2xl font-black text-agp-ink">Suas partidas</h2>
			<p class="text-agp-muted">Todas as partidas em que você aparece, incluindo pendentes.</p>
		</div>
		<div class="divide-y divide-agp-border">
			{#each myMatches as match}
				<a
					class="grid gap-4 px-5 py-5 transition hover:bg-agp-green-soft/40 lg:grid-cols-[1fr_9rem_8rem_10rem] lg:items-center lg:px-8"
					href={`/matches/${match.id}`}
				>
					<div class="flex items-center gap-4">
						<span
							class="flex h-12 w-12 items-center justify-center rounded-full bg-agp-green-soft font-black text-agp-green"
							>{initial(opponentId(match))}</span
						>
						<div>
							<strong class="text-xl text-agp-ink">vs {name(opponentId(match))}</strong>
							<p class="text-sm text-agp-muted">
								{match.winnerMemberId === data.member.id
									? 'Você venceu'
									: `${name(match.winnerMemberId)} venceu`} · {formatDate(match.playedAt)}
							</p>
						</div>
					</div>
					<strong class="w-fit rounded-2xl bg-stone-100 px-4 py-2 font-serif text-xl"
						>{match.scoreText}</strong
					>
					<span
						class="font-bold {match.status === 'pending'
							? 'text-orange-600'
							: match.status === 'confirmed'
								? 'text-agp-green'
								: 'text-red-600'}">{statusLabel(match.status)}</span
					>
					<span class="text-sm font-bold text-agp-muted"
						>{match.submittedByUserId === data.member.userId
							? 'Enviada por você'
							: 'Aguardando você'}</span
					>
				</a>
			{:else}
	...
	<div class="grid gap-10 xl:grid-cols-[1fr_1fr] xl:gap-12">
		<div class="space-y-8">
			<div class="flex flex-wrap items-end justify-between gap-4">
				<div>
		...
		<div class="space-y-8">
			...
			{#each data.matches.filter((m) => m.status === 'pending').slice(0, 6) as match}
