...
export async function acceptDisputedMatch(matchId: string, user: User) {
	return db.transaction(async (tx) => {
		const match = await tx.query.matches.findFirst({ where: eq(matches.id, matchId) });
		if (!match) error(404, 'Partida não encontrada.');
		if (match.status !== 'disputed' || match.submittedByUserId !== user.id)
			error(403, 'Você não pode aceitar esta contestação.');
		...
		if (!playerOne || !playerTwo) error(404, 'Jogadores da partida não encontrados.');
		...
		const updated = await tx
			...
			.where(and(eq(matches.id, matchId), eq(matches.status, 'disputed')))
			.returning();
		if (updated.length === 0) return match;
