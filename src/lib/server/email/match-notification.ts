...
export async function sendMatchConfirmationEmail(input: {
	...
	const result = await resend.emails.send({
		...
		html: `
			<div style="margin:0;background:#f8f6ef;padding:32px;font-family:Inter,Arial,sans-serif;color:#111821">
				<div style="margin:0 auto;max-width:620px;border:1px solid #ded8cb;border-radius:28px;background:#fffdf9;overflow:hidden">
					<div style="background:linear-gradient(135deg,#e6efe4,#fffdf9);padding:32px">
						<p style="margin:0 0 12px;color:#176b25;font-weight:800;text-transform:uppercase;letter-spacing:.08em">AGP</p>
						<h1 style="margin:0;font-family:Georgia,serif;font-size:40px;line-height:1;color:#111821">Resultado esperando tua confirmação</h1>
					</div>
					<div style="padding:32px">
						...
						<a href="${matchUrl}" style="display:block;text-align:center;background:#176b25;color:white;text-decoration:none;border-radius:18px;padding:16px 22px;font-weight:900;font-size:18px">Confirmar ou contestar resultado</a>
						<p style="font-size:14px;color:#5b5f68;margin:24px 0 0">Se o botão não abrir, acesse: <a style="color:#176b25" href="${matchUrl}">${matchUrl}</a></p>
					</div>
				</div>
			</div>
		`
