...
export const matches = pgTable('matches', {
	id: uuid('id').primaryKey().defaultRandom(),
	leagueId: uuid('league_id').notNull().references(() => leagues.id),
	playerOneMemberId: uuid('player_one_member_id').notNull().references(() => leagueMembers.id),
	playerTwoMemberId: uuid('player_two_member_id').notNull().references(() => leagueMembers.id),
	winnerMemberId: uuid('winner_member_id').notNull().references(() => leagueMembers.id),
	...
	differentPlayers: check('matches_different_players', sql`${table.playerOneMemberId} <> ${table.playerTwoMemberId}`),
	validWinner: check('matches_valid_winner', sql`${table.winnerMemberId} in (${table.playerOneMemberId}, ${table.playerTwoMemberId})`),
	validStatus: check('matches_valid_status', sql`${table.status} in ('pending', 'confirmed', 'disputed', 'cancelled')`)
}));
...
export const ratingEvents = pgTable('rating_events', {
	...
	opponentMemberId: uuid('opponent_member_id').notNull().references(() => leagueMembers.id),
...
export const usersRelations = relations(users, ({ many }) => ({ memberships: many(leagueMembers) }));
export const leaguesRelations = relations(leagues, ({ many }) => ({ members: many(leagueMembers), matches: many(matches) }));
